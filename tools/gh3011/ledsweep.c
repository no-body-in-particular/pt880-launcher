...
int main(void)
{
    ...
    printf("register  value   dc1        dc2        ac1    ac2\n");
    for (ri = 0; ri < 3; ri++) {
        unsigned short orig = 0;
        rd16(regs[ri], &orig);
        for (vi = 0; vi < 3; vi++) {
            double d1=0, d2=0, a1=0, a2=0;
            int got;
            wr16(regs[ri], vals[vi]);
            sample_once(&d1, &d2, &a1, &a2);          /* discard the transitional burst */
            got = sample_once(&d1, &d2, &a1, &a2);
            printf("  %04x    %04x   %-10.0f %-10.0f %-6.0f %-6.0f%s\n",
                   regs[ri], vals[vi], d1, d2, a1, a2, got ? "" : "   (no data)");
        }
        wr16(regs[ri], orig);                          /* put it back before the next register */
        printf("  %04x    restored to %04x\n\n", regs[ri], orig);
    }
